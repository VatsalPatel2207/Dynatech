...
$PasswordNotificationWindowInDays = @(40)
#---
...
$mailBody += "<p>We would like to remind you that your password for ($($user.UserPrincipalName)) is set to expire in <b>$($user.DaysRemaining) days</b>."
# $mailBody += "<p>AACN password for ($($user.UserPrincipalName)) will expire on <b>$(get-date $user.ExpiresOn -Format D)</b>." 
$mailBody += "<br>For security reasons, it is important to change your password every 90 days. To ensure uninterrupted access to your account, please take a moment to update your password at your earliest convenience.</p>" 
$mailBody += "<p>To change your password, please follow these steps:</p>"
$mailBody += "<p>1. Log in to your account here: https://myaccount.microsoft.com/</p>"
$mailBody += "<p>2. Click on the CHANGE PASSWORD link in the password box section.</p>"
$mailBody += "<p>3. Follow the prompts to create a new password.</p>"
$mailBody += "<p>4. If you are connected to the VPN, please disconnect and reconnect after changing the password.</p>"
$mailBody += "<p>5. If you are using the VPN Client on your AACN provided PC, don't forget to lock your PC and unlock it with the new password once you're reconnected to the VPN.  (Ctrl Alt Del - Lock)</p>"

$mailBody += "<p>If you encounter any issues or require assistance, please do not hesitate to contact our support team at 949-268-7500 or at 570@aacn.org.</p>"

$mailBody += "<p>Thank you for your prompt attention to this matter.</p>"

$mailBody += "<p>Best regards,</p>"
$mailBody += "<p>Your HelpDesk Team</p>"

$senderName = "AACN Account Services"
